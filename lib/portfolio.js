...
function calcWeights(combos, projectedReturns, volatility) {
  return combos.map(combo => {
    let weightedReturn = combo.split(',').reduce((acc, cur, idx) => {
      return acc + (cur / 100) * projectedReturns[idx];
    }, 0);
    let weightedVolatility = combo.split(',').reduce((acc, cur, idx) => {
      return acc + (cur / 100) * volatility[idx];
    }, 0);
    return `${combo},${weightedReturn.toFixed(2)},${weightedVolatility.toFixed(2)}`;
  });
}
...
function sortPortfolios(portfolios, numBuckets) {
  const portObjs = portfolios.map(p => {
    ...
    return {
      ...
      weighteVolatility: parseFloat(values[numBuckets + 1]),
      diversification: parseFloat(values[numBuckets + 2]),
    };
  });
  const sorted = _.sortBy(portObjs, ['weightedReturn', 'diversification', 'weighteVolatility']);
...
function generate(input) {
  ...
  const outFile = `data/out/${input.outputFilePrefix}-${Date.now()}.csv`;
  saveCsv(
    sortedPortfolios,
    outFile,
