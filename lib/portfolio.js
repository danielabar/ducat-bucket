...
function calcDiversification(portfolios, numBuckets, divMinMax) {
  return portfolios.map(portfolio => {
    ...
    const scaledDiversificationFinal = Math.abs(divMinMax.scaleMax - scaledDiversification);
...
function generate(input) {
  ...
  saveCsv(
    sortedPortfolios,
    outFile,
    `${assetClasses},Weighted Return,Weighted Risk (1 Low - 10 High),Diversification (1 Low - 10 High)`
  );
  console.log(
